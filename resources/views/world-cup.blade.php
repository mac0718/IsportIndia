    ...
    <div class="row">
      <section class="style_homeArticles__4e_Na " id="latestUpdateSection">
        <div class="line-title mb-2 mb-md-4 text-uppercase text-center overflow-hidden">
          <h4 class="rounded-pill position-relative d-inline-block">IPL</h4>
        </div>
        @if ($worldcup)
        @foreach($worldcup as $lh)
...
<script>
  ...
  function loadMoreLatest() {
    ...
    xhr.open('GET', '{{ route("loadMoreWorldCup") }}?offset=' + offset + '&limit=' + limit, true);
    ...
    xhr.onload = function() {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        response.worldcup.forEach(function(article) {
          var articleHTML = '';
