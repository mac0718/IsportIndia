...
$token = '6f733e21e9182c61e5310d8461319102';
function token()
{
...
function test()
{
	...
	$guzzleRequest = new GuzzleRequest('GET', 'https://rest.entitysport.com/exchange/teams/7/matches?token=' . $token);
...
{
	$today = Carbon::now();
	$fiveDaysAfter = $today->addDays(5);
	$todayFormatted = $today->format('Y-m-d');
	$fiveDaysAfterFormatted = $fiveDaysAfter->format('Y-m-d');
	$odi_match = Http::get(config('services.api') . 'matches/?status=1&format=1,2,3,4,6&date=' . $todayFormatted . '_' . $fiveDaysAfterFormatted . '&token=' . token())->json();
	$test_match = Http::get(config('services.api') . 'matches/?status=1&format=2,3,4,6&date=' . $todayFormatted . '_' . $fiveDaysAfterFormatted . '&token=' . token())->json();
	$t20_match = Http::get(config('services.api') . 'matches/?status=1&format=3,4,6&date=' . $todayFormatted . '_' . $fiveDaysAfterFormatted . '&token=' . token())->json();
	$listA_match = Http::get(config('services.api') . 'matches/?status=1&format=4,6&date=' . $todayFormatted . '_' . $fiveDaysAfterFormatted . '&token=' . token())->json();
	$ligT20_match = Http::get(config('services.api') . 'matches/?status=1&format=6&date=' . $todayFormatted . '_' . $fiveDaysAfterFormatted . '&token=' . token())->json();
...
function getAllCompletedMatches()
{
	$today = Carbon::now();
	$fiveDaysAgo = $today->subDays(5);
	$todayFormatted = $today->format('Y-m-d');
	$fiveDaysAgoFormatted = $fiveDaysAgo->format('Y-m-d');
	$odi_match = Http::get(config('services.api') . 'matches/?status=2&format=1,2,3,4,6&date='. $fiveDaysAgoFormatted. '_'. $todayFormatted .'&token=' . token())->json();
	$test_match = Http::get(config('services.api') .'matches/?status=2&format=2,3,4,6&date=' . $fiveDaysAgoFormatted . '_' . $todayFormatted . '&token=' . token())->json();
	$t20_match = Http::get(config('services.api') .'matches/?status=2&format=3,4,6&date=' . $fiveDaysAgoFormatted . '_' . $todayFormatted . '&token=' . token())->json();
	$listA_match = Http::get(config('services.api') .'matches/?status=2&format=4,6&date=' . $fiveDaysAgoFormatted . '_' . $todayFormatted . '&token=' . token())->json();
	$ligT20_match = Http::get(config('services.api') .'matches/?status=2&format=6&date=' . $fiveDaysAgoFormatted . '_' . $todayFormatted . '&token=' . token())->json();
